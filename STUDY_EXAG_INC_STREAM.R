...
library(hunspell)
#Load data
aire.data.stream<-read.csv('stream.csv', stringsAsFactors = FALSE,header=TRUE,sep=",",na.strings = c(""))
######## DATA WORD CLEANING ############
#Aire data tranformtion first: lower case, white space, remove numbers
aire.data.stream$emp_title_mod<-aire.data.stream$emp_title
aire.data.stream$emp_title_mod<-sapply(aire.data.stream$emp_title_mod,tolower)
aire.data.stream$emp_title_mod<-sapply(aire.data.stream$emp_title_mod,function(x) {gsub('[[:digit:]]+', '',x)})
#remove .,/&-@
aire.data.stream$emp_title_mod<-sapply(aire.data.stream$emp_title_mod,function(x) {gsub("[^[:alnum:]]"," ",x)})

aire.data.stream$emp_title_mod<-paste(" ",aire.data.stream$emp_title_mod," ")

#stopwords and acronym (obvious ones)
aire.data.stream$emp_title_mod<-gsub(" of "," ",aire.data.stream$emp_title_mod)
aire.data.stream$emp_title_mod<-gsub(" to "," ",aire.data.stream$emp_title_mod)
aire.data.stream$emp_title_mod<-gsub(" and "," ",aire.data.stream$emp_title_mod)
aire.data.stream$emp_title_mod<-gsub(" in "," ",aire.data.stream$emp_title_mod)
aire.data.stream$emp_title_mod<-gsub(" for "," ",aire.data.stream$emp_title_mod)

aire.data.stream$emp_title_mod<-gsub(" sr "," senior ",aire.data.stream$emp_title_mod)
aire.data.stream$emp_title_mod<-gsub(" vp "," vice president ",aire.data.stream$emp_title_mod)
aire.data.stream$emp_title_mod<-gsub(" v p "," vice president ",aire.data.stream$emp_title_mod)
aire.data.stream$emp_title_mod<-gsub(" admin "," administrative ",aire.data.stream$emp_title_mod)
aire.data.stream$emp_title_mod<-gsub(" dir "," director ",aire.data.stream$emp_title_mod)

repeat{
  aire.data.stream$emp_title_mod<-sapply(aire.data.stream$emp_title_mod,function(x) {gsub("  "," ",x)})
  if (length(aire.data.stream[grepl("  ",aire.data.stream$emp_title_mod),"emp_title_mod"]) == 0) {break}
}

#Extract all emp_title (or 5 characters at least)
utitles_new<-NULL
utitles_new<-sort(unique(aire.data.stream[nchar(aire.data.stream$emp_title_mod) > 4,"emp_title_mod"]), decreasing=FALSE)
#Check for misspell 
word<-NULL
misspell.words<-NULL
...
write.csv(misspell.words.and.corr.df, file = "misspell_emp_title_stream_4l.csv", na="")
...
#or
misspell.words.and.corr.df.read<-read.csv('misspell_emp_title_stream_4l.csv', stringsAsFactors = FALSE,header=TRUE,sep=",",na.strings = c(""))
...
#length(misspell.corr1) -->5636

aire.data.stream$need.corr<-0
for (i in 1:56)
{
  min=(i-1)*100+1
  max=i*100
  aire.data.stream[aire.data.stream$need.corr == 0 & grepl(paste(misspell.words[min:max], collapse='|'),aire.data.stream$emp_title_mod),"need.corr"]<-1
  print(paste("done [",min, ":" ,max, "] :",length(aire.data.stream[aire.data.stream$need.corr == 1,c("X")])))
}
aire.data.stream[aire.data.stream$need.corr == 0 & grepl(paste(misspell.words[5601:5636], collapse='|'),aire.data.stream$emp_title_mod),"need.corr"]<-1

#write.csv(aire.data.stream, file = "aire.stream.dev.csv", na="")

#aire.data.stream[grepl("vice ", aire.data.stream$emp_title_mod)==FALSE & grepl("vice_", aire.data.stream$emp_title_mod)==TRUE ,"emp_title_mod"]
...
aire.data.stream$emp_title_mod<-gsub(" vice "," vice_",aire.data.stream$emp_title_mod)

aire.data.stream$emp_title_mod.corr<-aire.data.stream$emp_title_mod
aire.data.stream$emp_title_mod.cont<-aire.data.stream$emp_title_mod
aire.data.misspell<-NULL
aire.data.misspell<-aire.data.stream[aire.data.stream$need.corr == 1,c("X","emp_title_mod","emp_title_mod.cont","emp_title_mod.corr")]
...
aire.data.all<-merge(aire.data.stream, aire.data.misspell, by.x=c("X"), by.y=c("X2"), all.x=TRUE) 
...
all.2wd.emp_title_n.order<-all.2wd.emp_title_n.order[nchar(all.2wd.emp_title_n.order$biwords)>5,]
...
#How many not found in dictionnary
length(aire.data.all[grepl("Unknown",aire.data.all$JobTitle),"JobTitle"])
#How many not specifed
length(aire.data.all[grepl("not_specified",aire.data.all$JobTitle),"JobTitle"])
...
#770 job title; unknown=20115; not specified=29481

#Left to do: acronyms, mirror biwords, stem

PlotCategoryData("JobTitle",aire.data.all)
write.csv(aire.data.all, file = "aire.data.stream.withJobTitle.csv", na="")
