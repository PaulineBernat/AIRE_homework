...
for (i in 1:nrow(aire.data.stream))
{
  if (aire.data.stream[i,"hasJob"] == 1 & aire.data.stream[i,"hasInc"] == 1 & aire.data.stream[i,"hasState"] == 1 & !(aire.data.stream[i,"job_title"] %in% c("not_specified", "unknown", "unclear")))
  {
    if(length(state.inc.job[grepl(aire.data.stream[i,"job_title"],state.inc.job$JobTitle) == TRUE & grepl(aire.data.stream[i,"addr_state"],state.inc.job$addr_state) == TRUE,"median.inc"])>0)
    {
      med_salary<-NULL
      n_jobs<-NULL
      act_inc = 0
      threshold <- 0
      med_salary <- state.inc.job[grepl(aire.data.stream[i,"JobTitle"],state.inc.job$JobTitle) == TRUE & grepl(aire.data.stream[i,"addr_state"],state.inc.job$addr_state) == TRUE,"median.inc"]
      sd_salary <- state.inc.job[grepl(aire.data.stream[i,"JobTitle"],state.inc.job$JobTitle) == TRUE & grepl(aire.data.stream[i,"addr_state"],state.inc.job$addr_state) == TRUE,"sd.inc"]
      n_jobs <- state.inc.job[grepl(aire.data.stream[i,"JobTitle"],state.inc.job$JobTitle) == TRUE & grepl(aire.data.stream[i,"addr_state"],state.inc.job$addr_state) == TRUE,"n"]
      if (nrow(med_salary) == 1) {threshold <- med_salary+(2*sd_salary)
      if (n_jobs > 20 & aire.data.stream[i,"annual_inc"] > threshold)
      {
        print(paste("Application ",i," flag for potential exaggerated income"))
        print(paste("State = ",aire.data.stream[i,"addr_state"],", Job = ",aire.data.stream[i,"JobTitle"],", inc =  ",aire.data.stream[i,"annual_inc"], " [vs med(",n_jobs,") = ",med_salary,"]"))
        count_event_exag<-(count_event_exag+1)
      }
      }
    }
...
#Check how many flag:
count_event_exag
#~3-4% of aire.data.stream application
